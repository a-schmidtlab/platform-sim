...
class DynamicPlatformSimulator:
    ...
    def _setup_main_visualization(self):
        ...
        # Create platform square as polygon (easier to rotate)
        ...
        square_corners = np.array([
            ...
            [platform_size, platform_size],
            [-platform_size, platform_size],
            [-platform_size, -platform_size]  # Close the square
        ])
        
        self.platform_patch, = self.ax_main.plot(
            square_corners[:, 0], square_corners[:, 1],
            color=self.config['platform_color'], 
            linewidth=3, alpha=self.config['platform_alpha'],
            marker='o', markersize=4, markerfacecolor=self.config['platform_color']
        )
    ...
    def _update_platform_square(self, x: float, y: float, psi: float):
        ...
        self.platform_patch.set_data(corners_global[:, 0], corners_global[:, 1])
